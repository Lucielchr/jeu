export class Hub extends Phaser.Scene { // Déclare la classe "Hub" qui hérite des fonctionnalités d'une scène Phaser
    ...
    afficherBulleIntroduction(message) {
        // Crée un conteneur (un groupe vide) placé juste au-dessus du joueur (-60 pixels en Y)
        // Cela permet de déplacer le texte et le fond en même temps
        let bulleConteneur = this.add.container(this.player.x, this.player.y - 60);

        // Crée l'objet texte avec le message choisi
        let texte = this.add.text(0, 0, message, {
            fontSize: '14px',        // Taille de la police
            fill: '#000000',         // Couleur du texte (noir)
            wordWrap: { width: 180 }, // Le texte revient à la ligne automatiquement après 180 pixels
            align: 'center'          // Texte centré dans sa zone
        }).setOrigin(0.5);           // Centre le point de pivot du texte pour qu'il soit bien au milieu

        // Crée un objet graphique pour dessiner le rectangle blanc derrière le texte
        let bg = this.add.graphics();
        bg.fillStyle(0xffffff, 1);   // Couleur de remplissage (blanc)
        bg.lineStyle(2, 0x000000, 1); // Bordure de 2 pixels (noire)

        // Dessine un rectangle arrondi calculé selon la taille du texte (+10 pixels de marge)
        ...
        // Ajoute le fond et le texte dans le conteneur
        bulleConteneur.add([bg, texte]);
        // Met la bulle au premier plan (profondeur 100) pour qu'elle passe au-dessus des décors
        bulleConteneur.setDepth(100);

        // Crée une boucle qui se répète toutes les 10ms pour que la bulle "colle" au joueur s'il bouge
        const suivreJoueur = this.time.addEvent({
            delay: 10,
            callback: () => {
                if (bulleConteneur) { // Vérifie que la bulle existe encore
                    bulleConteneur.x = this.player.x;      // Aligne X sur le joueur
                    bulleConteneur.y = this.player.y - 60; // Garde la bulle 60px au-dessus
                }
            },
            loop: true
        });

        // Programme la destruction de la bulle après 10 secondes (10000ms)
        this.time.delayedCall(10000, () => {
            suivreJoueur.remove(); // Arrête la boucle de suivi
            if (bulleConteneur) bulleConteneur.destroy(); // Supprime visuellement la bulle
        });
    }
